...
origins = [
    "http://localhost:3000",
    "https://credlift-amot4j5ue-sujaan-iqbals-projects.vercel.app",
]
...
# ==========================
# 6. CREATE GLOBAL OFFER (No customer_id required)
# ==========================
@app.post("/offers", status_code=201)
def create_offer(offer: schemas.Offer_Create, db: Session = Depends(get_db)):
    new_offer = Offer(
        lender=offer.lender,
        amount=offer.amount,
...
# ==========================
# 7. LIST OFFERS (Global + Score-Gated)
# ==========================
...
def list_offers(customer_id: int, locked: Optional[str] = Query(None), db: Session = Depends(get_db)):
    ...
    # Fetch ALL global offers (no customer_id filter)
